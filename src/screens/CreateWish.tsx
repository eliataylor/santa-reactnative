...
class CreateWish extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      !nextProps.entity.errors &&
      nextProps.entity.apiData.title === this.state.title &&
      this.state.title != ''
    ) {
      // WARN: a safe assumption, but not ideal key
  ...
  componentDidUpdate(prevProps) {
    if (
      !prevProps.entity.errors &&
      prevProps.entity.apiData.title === this.state.title &&
      this.state.title != ''
    ) {
      // WARN: a safe assumption, but not ideal key
  ...
  render() {
    ...
    if (this.props.auth.categories) {
      for (var i = 0; i < this.props.auth.categories.length; i++) {
        catOpts.push({
          label: this.props.auth.categories[i].name,
          value: this.props.auth.categories[i]._id,
        });
      }
    }
          ...
          <View style={styles.form}>
            ...
            <View>
              <LocationSelector
                ...
                onGpsLocation={this.props.updateLocation}
              />
            </View>
          </View>
